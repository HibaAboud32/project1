...
import styles from "./updateOpenMarket.module.css";
import Loader from "../../../loader/Loader";

const token = localStorage.getItem("find_me_token");

const UpdateOpenMarket = ({
  ...
  const [name_en, setName_en] = useState(
    `${openShop[indexUpdateProduct].name_en}`
  );
  const [name_ar, setName_ar] = useState(
    `${openShop[indexUpdateProduct].name_ar}`
  );
  const [description_en, setDescription_en] = useState(
    `${openShop[indexUpdateProduct].description_en}`
  );
  const [description_ar, setDescription_ar] = useState(
    `${openShop[indexUpdateProduct].description_ar}`
  );
  const [image, setImage] = useState("");
  const [price, setPrice] = useState(
    `${openShop[indexUpdateProduct].current_price}`
  );
  const [phone, setPhone] = useState(`${openShop[indexUpdateProduct].phone}`);
  const [location, setLocation] = useState(
    `${openShop[indexUpdateProduct].location}`
  );
  ...
  async function addProducte(e) {
    ...
    await axios
      .post(
        `${process.env.REACT_APP_API_URI}api/update_product/${state}`,
        formData,
        {
          headers: {
      ...
      <div className={styles.form_add_product}>
        ...
        <div className={`${styles.add_product_container} container`}>
          <div
            ...
            onClick={() => setOpenUpdateProduct(!openUpdateProduct)}
          >
            X
          </div>
          ...
          {/* errormessage */}
          <p
            className={errorMsg ? styles.error_msg : styles.offScreen}
            aria-live="assertive"
          >
            {errorMsg}
          </p>
            ...
            <div className={styles.input_container}>
              ...
              >
                {cities.map((element, index) => (
                  <option className={styles.option} key={index} value={element}>
                    {element}
                  </option>
                ))}
              </select>
...
export default UpdateOpenMarket;
